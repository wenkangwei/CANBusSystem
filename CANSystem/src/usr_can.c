...
void usr_CAN_Config()
{
    ...
    usr_CanTxMessage.StdId=0x0002;
    usr_CanTxMessage.IDE=CAN_ID_STD;
    usr_CanTxMessage.RTR=CAN_RTR_Data;			//if RTR is remote bits, then CAN won't send Data bits
    for(i=0;i<8;i++){

    	//clean buffer data
    	usr_CanRxMessage.Data[i]=0;
    	usr_CanTxMessage.Data[i]='a'+i;
    	usr_CanTxData[i]=0;
    	usr_CanRxData[i]=0;
    }
